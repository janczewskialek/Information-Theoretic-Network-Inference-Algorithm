...
void add_noise_timeseries_v0(Eigen::MatrixXd& mat,  int power);
...
double TE_X_to_Y(Eigen::MatrixXd X,
                 ...
                 int X_hist=1, int Y_hist=1,
                 int X_tau=1,  int Y_tau=1,
                 bool AUTOEMB=0, bool AUTODEL=0, int DELAY=1,
                  int permutation = 100,  double min_pval=0.20,
...
double CTE_X_to_Y(Eigen::MatrixXd X, Eigen::MatrixXd Y, Eigen::MatrixXd Z,
                  int X_hist=1,  int Y_hist=1, Eigen::MatrixXd Z_hist=Eigen::MatrixXd::Constant(1,1,1),
                   ...
                   int DELAY_X=1,  Eigen::MatrixXd DELAY_Z=Eigen::MatrixXd::Constant(1,1,1),
                   int permutation = 100,  double min_pval=0.20,
                   bool STD=0,  int NOISE=0,  int k=4,  int leaves=100);
...
returnVector_CTE CTE_X_to_Y_INA2(Eigen::MatrixXd X, Eigen::MatrixXd Y, Eigen::MatrixXd Z,
                      int X_hist=1,  int Y_hist=1, Eigen::MatrixXd Z_hist=Eigen::MatrixXd::Constant(1,1,1),
                      int X_tau=1,   int Y_tau=1,  Eigen::MatrixXd Z_tau=Eigen::MatrixXd::Constant(1,1,1),
                     bool AUTODEL = 1, int MAX_DELAY=1, int DELAY_X=1,  Eigen::MatrixXd DELAY_Z=Eigen::MatrixXd::Constant(1,1,1),
                      int permutation = 100,  double min_pval=0.20,
                      bool STD=0,  int NOISE=0,  int k=4,  int leaves=100);
...
returnVector_INA Information_Network_Analysis(Eigen::MatrixXd& dY,
                         ...
                         int X_tau  = 1,  int Y_tau  = 1,
                         int X_delay  = 1,
                         int permutation = 100,  double min_pval_TE=0.20,  double min_pval_CTE=0.20,
