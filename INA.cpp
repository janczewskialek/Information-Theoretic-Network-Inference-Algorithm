...
Eigen::MatrixXd CKD_find_KNN4_radius_per_point(Eigen::MatrixXd &Stack, const int k, const int leaves){
    ...
    std::vector<ckdtreenode> *tree_buffer = (std::vector<ckdtreenode>*) malloc(100000*sizeof(std::vector<ckdtreenode>));
...
Eigen::MatrixXd CKD_count_NNs_with_radius_search(Eigen::MatrixXd& Y1Y_stack, Eigen::MatrixXd& radiuses, const int leaves) {
    using namespace Eigen;
    ...
    std::vector<intptr_t> *results =(std::vector<intptr_t>*) malloc(1000000*sizeof(std::vector<intptr_t>)); // SIZE MATTER!
...
// function that adds noise to time serieses with specified scale
void add_noise_timeseries_v0(Eigen::MatrixXd& mat, const int power){
    using namespace Eigen;
    ...
    mat += noise_scale * gaussian;
}


// function that adds noise to time serieses with specified scale -- changed to align approach from JIDT
...
std::tuple<int, int> Ragwitz_auto_embedding(Eigen::MatrixXd X, Eigen::MatrixXd Y, const int k, const int k_max, const int tau_max){
    ...
    for (int k_candidate = 1; k_candidate <= k_max; k_candidate++){
        for (int tau_candidate = 1; tau_candidate <= tau_max; tau_candidate++){
            ...
            for(int t=0; t<Y_shift_cols; t++){
                ...
                value_to_predict = Y_shift.block(0,t,1,1); // destination value that is being predicted by embedded stack
...
double TE_X_to_Y_inner(Eigen::MatrixXd& X, Eigen::MatrixXd& Y,
    ...
    double TE_XtoY_Kraskov = (digamma_k - digamma_Y1Y - digamma_XY + digamma_Y ) / log(base); // logbase defines the units of the metric
...
// old version
double TE_X_to_Y(Eigen::MatrixXd X,
                 Eigen::MatrixXd Y,
                 int X_hist, int Y_hist,
                 int X_tau,  int Y_tau,
                 bool AUTOEMB, bool AUTODEL, int DELAY,
                 const int permutation, const double min_pval,
    ...
    if (AUTOEMB){
        ...
        std::tuple k_tau_destination = Ragwitz_auto_embedding(X, Y,k, Y_hist, Y_tau);
        ...
        std::tuple k_tau_source = Ragwitz_auto_embedding(X, X,k, X_hist, X_tau);
    ...
    int delay_best;
    if (AUTODEL){
        for (int delay_candidate=1; delay_candidate<= DELAY; delay_candidate++){
        ...
        // calculate value for the actual value of TE
...
double CTE_X_to_Y_inner(Eigen::MatrixXd& X, Eigen::MatrixXd& Y, Eigen::MatrixXd& Z,
    ...
    int main_shift = std::max({(X_hist-1)*X_tau+DELAY_X, (Y_hist-1)*Y_tau+DELAY_X, (Z_max_hist-1)*Z_max_tau+Z_max_delay});
    main_shift -=1;
...
//    MatrixXd Z_HIST_STACK = Z.block(0,0,Z.rows(),Y_shift_cols);
...
double CTE_X_to_Y_permute(Eigen::MatrixXd& X, Eigen::MatrixXd& Y, Eigen::MatrixXd& Z, double CTE_actual,
    ...
    for (int i =0; i< permutation; i++) {
        ...
        double CTE_XtoY_surrogate = (digamma_k - digamma_Y1Y - digamma_XY + digamma_Y ) / log(base); // logbase defines the units of the metric
...
double CTE_X_to_Y(Eigen::MatrixXd X, Eigen::MatrixXd Y, Eigen::MatrixXd Z,
                  const int X_hist, const int Y_hist, Eigen::MatrixXd Z_hist,
                  const int X_tau,  const int Y_tau,  Eigen::MatrixXd Z_tau,
                  const int DELAY_X,  Eigen::MatrixXd DELAY_Z,
                  const int permutation, const double min_pval,
    ...
    double CTEntropy_actual = CTE_X_to_Y_inner( X, Y, Z,
    ...
    if (permutation > 0){
        p_value = CTE_X_to_Y_permute( X, Y, Z, CTEntropy_actual,
                                      X_hist, Y_hist, Z_hist,
                                      X_tau, Y_tau, Z_tau,
                                      DELAY_X, DELAY_Z,
...
returnVector_CTE CTE_X_to_Y_INA(Eigen::MatrixXd X, Eigen::MatrixXd Y, Eigen::MatrixXd Z,
    ...
    // calculate value for the actual value of CTE
    double CTE_estimate = CTE_X_to_Y_inner( X, Y, Z,
                                            X_hist, Y_hist, Z_hist,
                                            X_tau, Y_tau, Z_tau,
                                            DELAY_X, DELAY_Z, k, leaves);
...
returnVector_CTE CTE_X_to_Y_INA2(Eigen::MatrixXd X, Eigen::MatrixXd Y, Eigen::MatrixXd Z,
                     const int X_hist, const int Y_hist, Eigen::MatrixXd Z_hist,
                     const int X_tau,  const int Y_tau,  Eigen::MatrixXd Z_tau,
...
returnVector_INA Information_Network_Analysis(Eigen::MatrixXd& dY,
                                  ...
                                  const int X_tau, const int Y_tau,
                                  const int X_delay,
                                  const int permutation, const double min_pval_TE, const double min_pval_CTE,
                                  const bool STD, const int NOISE, const int k, const int leaves, const bool DEBUG) {
    ...
    if (CTE_analysis) {
        ...
        for (int source = 0; source < nbOfDealers; source++) {
            for (int destination = 0; destination < nbOfDealers; destination++, counter++) {
                ...
                if (TE_pvalues(source,destination)==-1 || TE_pvalues(source,destination)>min_pval_TE){continue;}
                ...
                for (int candidate=0; candidate < nbOfDealers; candidate++){
                    ...
                    if (candidatesToCheck_pvalues(candidate,0) != -1 && candidatesToCheck_pvalues(candidate,0) <= min_pval_TE){
                        conditional_variables_indexes.push_back(candidate);
                    }
                }

                // TODO: Additional prunning that determines truly significant source when all considered
                ...
                if (conditional_variables_indexes.size() > 0){
                    ...
                    for (int i=0; i<nbOfConditionals; i++){
                        Z_CTE.block(i,0,1,nbOfDatapoints) = dY.block(conditional_variables_indexes[i], 0, 1, nbOfDatapoints);
                    ...
                    auto[CTE_results_temp, CTE_pvalues_temp] = CTE_X_to_Y_INA2(X_CTE, Y_CTE, Z_CTE,
                                                                               X_hist_mat(source,destination), Y_hist_mat(source,destination), Z_hist_mat,
                                                                               X_tau_mat(source,destination),  Y_tau_mat(source,destination), Z_tau_mat,
                                                                               AUTODEL_CTE, MAX_DELAY,
                                                                               DELAY_mat(source,destination),  DELAY_Z_mat,
