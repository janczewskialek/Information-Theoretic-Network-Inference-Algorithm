...
// PSEUDO VALIDATION OF INA TO BE MADE PUBLIC
void VALIDATION() {
    // Parameters
    int Y_hist = 1;
    int Y_tau = 1;
    ...
    std::ifstream file3;
    file3.open("Test_Data/test4d_6000p.txt");
    MatrixXd INA_test_data = MatrixXd::Zero(5999, 4);

    for (int i = 0; i < 5999; i++)
        for (int j = 0; j < 4; j++)
            file3 >> INA_test_data(i, j);
    file3.close();

    for (int i = 0; i < INA_test_data.rows(); ++i) {
        for (int j = 0; j < INA_test_data.cols(); ++j) {
        }
    }

    INA_test_data.transposeInPlace();

    std::cout << "\nTE VALIDATION IN PROGRESS\n" << std::endl;

    auto var = 1;

    auto [TE_results, TE_pvalues, CTE_results, CTE_pvalues]
            = Information_Network_Analysis(INA_test_data,
                                           CTE_analysis,
                                           AUTOEMB,
                                           MAX_HIST,
                                           MAX_TAU,
    ...
    if (TE_results(0, 1) > 0.0516913 && TE_results(0, 1) < 0.0516915 &&
        TE_results(0, 2) > 0.0190070 && TE_results(0, 2) < 0.0190072 &&
        TE_results(0, 3) > 0.0588504 && TE_results(0, 3) < 0.0588506 &&
        TE_results(1, 0) > 0.00525593 && TE_results(1, 0) < 0.00525595 &&
        TE_results(1, 2) > 0.083384 && TE_results(1, 2) < 0.083386) {
        std::cout << "- TE Validation Passed !" << std::endl;
    } else {
        std::cout << "- ERROR - Validation NOT Passed!!!!! " << std::endl;
    }

    std::cout << "\nCTE VALIDATION RESULTS: ";
    if (CTE_results(0, 1) > 0.0467075 && CTE_results(0, 1) < 0.0467077 &&
        CTE_results(0, 2) > 0.0238114 && CTE_results(0, 2) < 0.0238116 &&
        CTE_results(0, 3) > 0.0592686 && CTE_results(0, 3) < 0.0592688 &&
        CTE_results(1, 0) > 0.0048265 && CTE_results(1, 0) < 0.0048267 &&
        CTE_results(1, 2) > 0.0870180 && CTE_results(1, 2) < 0.0870182) {
        std::cout << "- CTE Validation Passed !" << std::endl;
    } else {
        std::cout << "- ERROR - Validation NOT Passed!!!!! " << std::endl;
    }
